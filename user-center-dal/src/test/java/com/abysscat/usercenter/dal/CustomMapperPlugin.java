...
/**
 * 框架自带的mapper plugin 各种写死的代码，直接重写
 */
public class CustomMapperPlugin extends FalseMethodPlugin {
    ...
    private void processEntityClass(TopLevelClass topLevelClass, IntrospectedTable introspectedTable) {
        ...
        if (generateColumnConsts) {
            for (IntrospectedColumn introspectedColumn : introspectedTable.getAllColumns()) {
                ...
                field.setFinal(true);
                field.setName("FIELD_" + introspectedColumn.getActualColumnName().toUpperCase()); //$NON-NLS-1$
        ...
        if (generateDefaultInstanceMethod) {
            ...
            defaultMethod.addBodyLine(String.format("%s instance = new %s();",
                    topLevelClass.getType().getShortName(), topLevelClass.getType().getShortName()));
            for (IntrospectedColumn introspectedColumn : introspectedTable.getAllColumns()) {
                String shortName = introspectedColumn.getFullyQualifiedJavaType().getShortName();
                List<String> supportType = Arrays.asList("Byte", "Short", "Character", "Integer",
                        "Long", "Float", "Double", "String", "BigDecimal", "BigInteger");
                ...
                if (introspectedColumn.getDefaultValue() != null) {
                    ...
                    // 通过 new 方法转换
                    defaultMethod.addBodyLine(String.format("instance.%s = new %s(\"%s\");",
                            introspectedColumn.getJavaProperty(), shortName, defaultValue));
                }
